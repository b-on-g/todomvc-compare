# TodoMVC: $mol vs Lit vs Symbiote.js vs Cruzo

Архитектурное сравнение подходов к построению UI на примере TodoMVC.
...
|              | hyoo_todomvc ($mol)    | Lit (Speedometer 3.0)        | Symbiote.js                     | Cruzo                            |
| ------------ | ---------------------- | ---------------------------- | ------------------------------- | -------------------------------- |
| **Путь**     | `hyoo/todomvc/`        | `bog/todomvc-compare/lit/`   | `bog/todomvc-compare/symbiote/` | `bog/todomvc-compare/cruzo/`     |
| **Источник** | Оригинал               | WebKit/Speedometer 3.0 `lit` | Написан с нуля                  | Написан с нуля                   |
| **Язык**     | TypeScript + view.tree | TypeScript                   | JavaScript                      | TypeScript                       |
...
| Метрика              | $mol                                          | Lit                        | Symbiote                  | Cruzo                           |
| -------------------- | --------------------------------------------- | -------------------------- | ------------------------- | ------------------------------- |
| **LOC логика + CSS** | 357 (134 .view.ts + 60 .view.tree + 163 .css) | 832 (502 логика + 330 CSS) | 566 (логика + CSS вместе) | 352 (логика, CSS в index.html)  |
| **LOC тесты**        | 140                                           | —                          | —                         | —                               |
| **LOC итого**        | 357 ( с тестами 497 )                         | 832                        | 566                       | 352                             |
| **Файлов (src)**     | 4 + css                                       | 10                         | 4                         | 5                               |

### Размер бандла

| Метрика            | $mol    | Lit   | Symbiote | Cruzo |
| ------------------ | ------- | ----- | -------- | ----- |
| **Bundle raw**     | 185 KB  | 36 KB | 27 KB    | 52 KB |
| **Bundle gzip**    | 33 KB   | 12 KB | 9 KB     | 15 KB |
| **Bundle brotli**  | 27 KB   | 11 KB | 8 KB     | 14 KB |
| **Фреймворк gzip** | ~29 KB  | ~8 KB | ~6 KB    | ~13 KB |
| **App-код gzip**   | ~4 KB   | ~4 KB | ~3 KB    | ~2 KB |
...
| Фича              | $mol                               | Lit       | Symbiote        | Cruzo                    |
| ----------------- | ---------------------------------- | --------- | --------------- | ------------------------ |
| **Shadow DOM**    | Нет                                | Да        | Нет (Light DOM) | Нет (Light DOM)          |
| **TypeScript**    | Да                                 | Да        | Нет             | Да                       |
| **Тесты без DOM** | Да (140 строк)                     | Нет       | Нет             | Нет                      |
| **Виртуализация** | Из коробки (все компоненты вообще) | Нет       | Нет             | Нет                      |
| **localStorage**  | Да ($mol_state_local)              | Нет       | Да              | Да                       |
| **Роутинг**       | Да ($mol_state_arg)                | Да (hash) | Да (hash)       | Да (hash)                |
| **Реактивность**  | Pull (автограф)                    | Push      | Push            | Push (Rx + стековая VM)  |
...
| --------------------------------- | ----------- | ------------------------------ |
| A. Память (124 vs 16 байт)        | НЕИСПРАВИМО | Lit, Symbiote, Cruzo           |
| B. JIT-потолок (host objects)     | НЕИСПРАВИМО | Lit, Symbiote, Cruzo           |
| C. Push vs Pull реактивность      | НЕИСПРАВИМО | Lit, Symbiote, Cruzo           |
| D. Поздний жизненный цикл         | НЕИСПРАВИМО | Lit, Symbiote, Cruzo           |
| E. Shadow DOM vs глобальные стили | Частично    | Lit (Symbiote, Cruzo обходят)  |
| F. Компиляция шаблонов            | Частично    | Lit, Symbiote, Cruzo           |
| G. Тестируемость без DOM          | НЕИСПРАВИМО | Lit, Symbiote, Cruzo           |
| H. Наследование шаблонов          | НЕИСПРАВИМО | Lit, Symbiote, Cruzo           |
...
Cruzo интересен встроенной стековой VM для шаблонных выражений и Rx-реактивностью, но не использует Custom Elements напрямую (компоненты — обычные классы, привязанные к DOM через `querySelectorAll`). Тем не менее, компонент без DOM-ноды не работает — те же фундаментальные ограничения.
